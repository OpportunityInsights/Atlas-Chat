...
If a user knows how to use the chat then generally they should be able to find what they are looking for. However, if someone is not familiar with the chat's workflow, then trying to get data, calculate statistics, or make figures can be confusing. To fix this, more explanatory text could be added to the chat's error messages and to the prompts given to chat-GPT.
...
Another thing that could make the chat better would be to replace the generic LLM powering it (in this case gpt-4o) with a fine tuned LLM. Fine tunning is when a developer continues the training of a model on a specific, usually relatively small, dataset. [This can be done through the OpenAI API.](https://platform.openai.com/docs/guides/fine-tuning/create-a-fine-tuned-model) Fine tuning the model could improve the quality of outputs, reduce costs, and make the chat faster. Here are three example of datasets that could be used in fine tuning:
1. User messages and chat responses from successful conversations with the Atlas Chat
2. Dialogue from preexisting interviews with Professor Chetty or other members of the Opportunity Insights team
3. Emails sent to OI in the past asking about data and the responses to those emails
