...
> [!WARNING]
> Their is a bug in the code. Your deployment will return an error when you try to make a map of census tracts.
...
`handle_chat_request_no_sheets()` then gets all the variable headers from all the sheets and returns a list with them ordered by the distance metric. The lower the index of a header, the closer the header is to the key words. The headers also contain information about the sheet they are from and the description of the variable. For example, one of the descriptions with an embedding and distanced could be "hs - the fraction of children who completed highschool". This would match with all the related headers like hs_pooled_pooled_mean, hs_black_pooled_mean, and hs_white_pooled_mean.
