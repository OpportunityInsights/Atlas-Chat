...
In this branch of the flow chart, `sendMessage()` starts by calling [fetchData()](https://github.com/xamxl/Atlas-Chat/blob/main/Flask/static/js/script.js#L849) which calls [/getRankedVariables](https://github.com/xamxl/Atlas-Chat/blob/main/Flask/main.py#L581) in the server with the key words the previous function call said should be used for the search. `/getRankedVariables` calls [getRankedVariables()](https://github.com/xamxl/Atlas-Chat/blob/main/Flask/main.py#L246) which gets the embedding values of the key words using the OpenAI API.

`getRankedVariables()` then fetches a set of precalculated embeddings from the database. Each variable in the database corresponds to one of these embeddings, but each one of these precalculated embeddings does not necessarily correspond to only one variable. This is because the precalculated embeddings were made with text like "hs - the fraction of children who completed high school." This embeddings would correspond to multiple variables including hs_black_female_mean and hs_white_female_mean.

After fetching these stored embeddings `getRankedVariables()` uses cosine similarly to find the distance between the embedding of the user's keywords and the stored embeddings. It then calculates another type of distance, dubbed "dumb distance." "Dumb distance" is the fraction of the keywords that appear in the text that was originally used to calculated the stored embeddings. An example of this text could be "hs - the fraction of children who completed high school."

The two different distance metrics are combined, weighing the cosine similarity by 0.8 and the "dumb distance" by 0.2. The cosine similarity embedding distance is used so that terms like "upward mobility" which do not appear in the variable descriptions can be matched with text from the descriptions with a similar meaning. The "dumb distance" is used so that terms like "individual" which appear in many descriptions but whose meaning isn't picked up by the embedding model can be exactly matched.

`getRankedVariables()` then gets all the variable headers from all the sheets and returns a list with them ordered by the distance metric that they match with. For example, one of the descriptions with an embedding and distance could be "hs - the fraction of children who completed high school". This would match with all the related headers like hs_pooled_pooled_mean, hs_black_pooled_mean, and hs_white_pooled_mean. The lower the index of a header, the closer the header is to the keywords. The headers are concatenated with information about the sheet they come from and the description of the variable.
