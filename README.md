...
To save money, the function call in `/useCase` is only given four messages to use when deciding which action to take. Since it's missing most of the context of the chat, the function call sometimes decides to take the wrong action. Since the OpenAI API has already gotten much cheaper, it may make sense to give `/useCase` more messages to use when deciding which action to take.
