...
In the base directory, flowchart.jpeg, structure.jpeg, and README.md are all files that help explain the code. The setup.py file contains code that was used to construct the database. The .gitignore file prevents these sensitive files from being uploaded to github. The Flask folder contains the application itself. When running the program, the whole Flask folder is part of the execution. Inside the Flask folder there are loose files and there other directories.
