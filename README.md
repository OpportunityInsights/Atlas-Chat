...
In this branch of the flow chart, `sendMessage()` starts by calling [fetchData()](https://github.com/xamxl/Atlas-Chat/blob/main/Flask/static/js/script.js#L849) which calls [/getRankedVariables](https://github.com/xamxl/Atlas-Chat/blob/main/Flask/main.py#L581) in the server with the key words the previous function call said should be used for the search. `/getRankedVariables` calls [getRankedVariables()](https://github.com/xamxl/Atlas-Chat/blob/main/Flask/main.py#L246). `getRankedVariables()` starts by finding the embedding values using the of the keywords the function call thought should be used in the search. To find the embedding values it uses the OpenAI API.

`getRankedVariables()` then gets a set of embeddings that correspond to a set of all variables in the database and their descriptions. Note that embeddings were not created and stored for variables like kfr_white_female_mean and kfr_black_female_mean. Instead, embeddings were created for titles like kfr. These embeddings were calculated once when the server code was written. They are not calculated every time `getRankedVariables()` runs. After fetching these stored embeddings the function finds the distance between the embedding of the user's keywords and the stored embeddings with cosine similarity. It then calculates another type of distance, dubbed "dumb distance." "Dumb distance" is the fraction of the keywords that appear in each of the descriptions of the variables in the database.
