...
`getRankedVariables()` then gets a set of embeddings that correspond to a set of all variables in the database and their descriptions. These embeddings were calculated once when the server code was written. They are not calculated every time `getRankedVariables()` runs. The function then calculates another type of distance, dubbed "dumb distance." "Dumb distance" is the fraction of the keywords that appear in each of the descriptions of the variables in the database.
